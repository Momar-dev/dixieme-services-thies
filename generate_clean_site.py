...
workers = [
    {
        ...
        'avatar_icon': 'fa-solid fa-person-digging',
    ...
    },
    {
        ...
        'phone_tertiary': '+221765448181',
        'phone_tertiary_display': '76 544 81 81',
        'meta_chips': [
...
def render_worker_card(w, index):
    ...
    if w.get('is_dual'):
        ...
        extra_phones_html = ''
        if w.get('phone_secondary') or w.get('phone_tertiary'):
            btns = []
            if w.get('phone_secondary'):
                btns.append(f'''<a href="tel:{w['phone_secondary']}" class="btn-secondary-call" onclick="showToast('Appel fixe {w['phone_secondary_display']}...')" title="Appeler le fixe"><i class="fa-solid fa-phone-volume"></i> {w['phone_secondary_display']}</a>''')
            if w.get('phone_tertiary'):
                btns.append(f'''<a href="tel:{w['phone_tertiary']}" class="btn-secondary-call" onclick="showToast('Appel {w['phone_tertiary_display']}...')" title="Appeler le {w['phone_tertiary_display']}"><i class="fa-solid fa-mobile-screen"></i> {w['phone_tertiary_display']}</a>''')
            extra_phones_html = f'''
                    <div class="worker-extra-phones">
                        {''.join(btns)}
                    </div>'''
...
w_start = template.find('<section id="workers"')
s_start = template.find('<section id="services"')
d_start = template.find('<section id="demarches"')

part1 = template[:w_start]
part4 = template[d_start:]

# Update stats in part1
part1 = re.sub(r'<div class="stat-number">28\+?</div>\s*<div class="stat-label">Artisans & Pros Vérifiés</div>',
               '<div class="stat-number">31</div>\n                <div class="stat-label">Professionnels Répertoriés</div>',
               part1)

workers_section_new = f'''<section id="workers" aria-labelledby="workers-heading">
...
# Update footer services list safely
old_services_list = '''                <h3>Services Clés</h3>
                <ul>
                    <li><a href="#services">Pharmacie de garde 24/7</a></li>
                    <li><a href="#workers">Artisans BTP & Maçonnerie</a></li>
                    <li><a href="#services">Livraison express par moto</a></li>
                    <li><a href="#services">Électricité & Plomberie</a></li>
                    <li><a href="#assistance">Encadrement scolaire</a></li>
                    <li><a href="#assistance">Permanence santé</a></li>
                </ul>'''

new_services_list = '''                <h3>Services Clés</h3>
                <ul>
                    <li><a href="#services">Pharmacie de garde</a></li>
                    <li><a href="#services">Immobilier</a></li>
                    <li><a href="#services">Livraison express</a></li>
                    <li><a href="#services">Plomberie</a></li>
                    <li><a href="#services">Électricité</a></li>
                    <li><a href="#services">Location de Voiture</a></li>
                </ul>'''

if old_services_list in part4:
    part4 = part4.replace(old_services_list, new_services_list)

final_html = part1 + workers_section_new + services_new_section + part4
